...
func HandleConnection(conn net.Conn, events chan *Event) {
	...
	for {
		...
		case byte(MsgWantHeartBeat):
			if clientIdentity == byte(MsgWantHeartBeat) { // can a camera or dispatcher ask for a heartbeat? If so, we shouldn't assign this as an identity
			...
			clientIdentity = byte(MsgWantHeartBeat)
